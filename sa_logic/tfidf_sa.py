    ...
    def baseline(self, model_name, list_classifier, analyzer='word', ngram= None, min_df=3,
        ...
        try:
            ...
            for classifier_name, classifier in models.items():
                ...
                vectorizer = TfidfVectorizer(analyzer=analyzer, lowercase=True, encoding='utf-8', min_df=min_df,
...
#     clean_data = True
#     tfidf_sa = TFIDFSA(clean_data=clean_data)
#     for i in range(1,3):
#         for ngram in range(2,5):
#             min_df = 3
#             model_name = 'TFIDF_N@'+ str(ngram) + '_df@' + str(min_df) + '_clean_data' if clean_data else '_raw_data'
#             list_classifier = ['SVM', 'MultinomialNB', 'LogisticRegression', 'RandomForest', 'MLPClassifier']
#             tfidf_sa.baseline(model_name=model_name,list_classifier=list_classifier, ngram=(1, ngram), min_df=3,
#                               max_features=5000, stop_words=True, fold=10, iteration=3)
